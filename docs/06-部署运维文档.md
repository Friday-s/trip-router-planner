...
# 高德地图配置（Web服务Key，用于后端路径规划API）
AMAP_WEBSERVICE_KEY=your_webservice_key
...
**重要提示**：
- MySQL 8.0+ 需要在 JDBC URL 中添加 `allowPublicKeyRetrieval=true`
- 如果使用空密码，设置 `DB_PASSWORD=`（等号后为空）
...
JS API Key 在 Vue 组件中使用 AMapLoader 时传入。

### 2.3 高德地图密钥说明

2021年12月后申请的 Key 需要配合安全密钥使用：

| 密钥类型 | 申请位置 | 用途 | 配置位置 |
|----------|----------|------|----------|
| Web服务Key | 高德开放平台-Web服务 | 后端路径规划API | 后端环境变量 |
| JS API Key | 高德开放平台-Web端(JS API) | 前端地图展示 | Vue组件代码 |
| 安全密钥 | 高德开放平台-应用管理 | 配合JS API Key | index.html |
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| `Public Key Retrieval is not allowed` | MySQL 8.0+ 认证问题 | JDBC URL 添加 `allowPublicKeyRetrieval=true` |
| `Access denied for user 'root'` | 密码配置不匹配 | 检查 application.yml 中的密码配置 |
| `Parameter index out of range` | JSON_CONTAINS 语法错误 | 修正 MyBatis 参数绑定方式 |
| 管理员无法登录 | BCrypt 哈希不匹配 | 重新生成正确的密码哈希 |
| 地图只显示标记点 | 坐标类型为字符串 | 前端使用 parseFloat() 转换 |
| 地图完全不显示 | 安全密钥未配置 | 配置 _AMapSecurityConfig |
| 行程生成超时 | AI API 问题 | 检查 DeepSeek API Key 和网络 |
