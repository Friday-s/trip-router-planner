...
2021年12月02日后申请的 Key 需要配合安全密钥使用：

| Key类型 | 用途 | 配置位置 | 暴露风险 |
|--------|------|---------|---------|
| JS API Key | 前端地图展示 | 前端代码 | 低（需配置域名白名单） |
| 安全密钥 | 配合 JS API Key 使用 | 前端代码（loader.js 之前） | 低 |
| Web服务Key | 后端调用路径规划API | 后端环境变量 | 高（必须保密） |

#### 前端集成方式

**推荐使用 AMapLoader 动态加载**（官方推荐）：

```html
<!-- index.html -->
<!-- 1. 安全密钥配置（必须在 loader.js 之前） -->
...
**注意事项**：
1. 安全密钥必须在 loader.js 之前配置
2. 后端返回的坐标如果是 BigDecimal，前端需要用 parseFloat() 转换
3. AMapLoader.load() 返回 Promise，便于异步控制
...
为支持从AI API到本地算法的平滑切换，设计统一的推荐引擎接口：
...
│                                                                          │
│   + generateItinerary(request, candidates) : ItineraryPlan              │
...
│ Engine             │ │ Engine             │ │ ationEngine        │
├────────────────────┤ ├────────────────────┤ ├────────────────────┤
│ • DeepSeek API     │ │ • 本地规则引擎     │ │ • AI + 规则混合    │
│ • 其他AI API       │ │ • 评分模型         │ │ • 自动降级         │
└────────────────────┘ └────────────────────┘ └────────────────────┘
      MVP阶段                v2.0阶段              可选
```
...
| 扩展点 | 当前实现 | 扩展方式 |
|-------|---------|---------|
| 推荐引擎 | AIRecommendationEngine | 实现 RecommendationEngine 接口 |
| 路径优化 | 最近邻算法 | 实现 RouteOptimizer 接口 |
| 评分模型 | 主题匹配 | 实现 ScoringStrategy 接口 |
| 缓存策略 | Redis | 实现 CacheProvider 接口 |
| 地图服务 | 高德地图 | 实现 MapService 接口 |
...
| 表 | 预留字段/表 | 用途 | 启用版本 |
|----|-----------|------|---------|
| pois | popularity_score | 热度评分 | v1.5 |
| pois | weather_suitable | 天气适配 | v1.5 |
| users | preferences | 用户偏好 | v2.0 |
| - | comments | 评论表 | v1.5 |
| - | user_behaviors | 行为记录 | v2.0 |
| - | recommendation_logs | 推荐日志 | v2.0 |

详细扩展设计请参考 [08-未来规划与扩展设计.md](./08-未来规划与扩展设计.md)
